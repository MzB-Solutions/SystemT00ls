...
namespace SystemT00ls.CoreLib.System.Power
{
    ...
    public class Control
    {
        ...
        /// <summary>
        /// Forces processes to terminate. When this flag is set, the system does not send the
        /// WM_QUERYENDSESSION and WM_ENDSESSION messages. This can cause the applications to lose
        /// data. Therefore, you should only use this flag in an emergency.
        /// </summary>
        private const int _eWX_FORCE = 4;
        ...
        /// </summary>
        /// <param name="_tokenHandle">
        /// Handle to the access token that contains the privileges to be modified. The handle must
        ...
        /// </param>
        /// <param name="_disableAllPrivileges">
        ...
        /// attributes. If the DisableAllPrivileges parameter is FALSE, AdjustTokenPrivileges
        /// enables or disables these privileges for the token. If you set the SE_PRIVILEGE_ENABLED
        /// attribute for a privilege, the function enables that privilege; otherwise, it disables
        /// the privilege. If DisableAllPrivileges is TRUE, the function ignores this parameter.
        /// </param>
        /// <param name="_bufferLength">
        ...
        /// </param>
        /// <param name="_previousState">
        /// Pointer to a buffer that the function fills with a TOKEN_PRIVILEGES structure that
        /// contains the previous state of any privileges that the function modifies. This parameter
        /// can be NULL.
        /// </param>
        /// <param name="_returnLength">
        /// Pointer to a variable that receives the required size, in bytes, of the buffer pointed
        ...
        /// adjusted all of the specified privileges, call Marshal.GetLastWin32Error.
        /// </returns>
        [DllImport("advapi32.dll", EntryPoint = "AdjustTokenPrivileges", CharSet = CharSet.Ansi)]
        private static extern int _adjustTokenPrivileges(IntPtr _tokenHandle, int _disableAllPrivileges, ref TOKEN_PRIVILEGES _newState, int _bufferLength, ref TOKEN_PRIVILEGES _previousState, ref int _returnLength);
        ...
        /// </summary>
        /// <param name="_uFlags">Specifies the type of shutdown.</param>
        /// <param name="_dwReserved">This parameter is ignored.</param>
        /// <returns>
        /// If the function succeeds, the return value is nonzero. <br></br><br>If the function
        /// fails, the return value is zero. To get extended error information, call Marshal.GetLastWin32Error.</br>
        /// </returns>
        [DllImport("user32.dll", EntryPoint = "ExitWindowsEx", CharSet = CharSet.Ansi)]
        private static extern int _exitWindowsEx(int _uFlags, int _dwReserved);

        /// <summary>
        /// The FormatMessage function formats a message string. The function requires a message
        /// definition as input. The message definition can come from a buffer passed into the
        /// function. It can come from a message table resource in an already-loaded module. Or the
        /// caller can ask the function to search the system's message table resource(s) for the
        /// message definition. The function finds the message definition in a message table
        /// resource based on a message identifier and a language identifier. The function copies
        /// the formatted message text to an output buffer, processing any embedded insert sequences
        /// if requested.
        /// </summary>
        /// <param name="_dwFlags">
        /// Specifies aspects of the formatting process and how to interpret the lpSource parameter.
        /// The low-order byte of dwFlags specifies how the function handles line breaks in the
        /// output buffer. The low-order byte can also specify the maximum width of a formatted
        /// output line.
        /// </param>
        /// <param name="_lpSource">
        /// Specifies the location of the message definition. The type of this parameter depends
        /// upon the settings in the dwFlags parameter.
        /// </param>
        /// <param name="_dwMessageId">
        ...
        /// </param>
        /// <param name="_dwLanguageId">
        /// Specifies the language identifier for the requested message. This parameter is ignored
        ...
        /// FORMAT_MESSAGE_ALLOCATE_BUFFER, the function allocates a buffer using the LocalAlloc
        /// function, and places the pointer to the buffer at the address specified in lpBuffer.
        /// </param>
        /// <param name="_nSize">
        /// If the FORMAT_MESSAGE_ALLOCATE_BUFFER flag is not set, this parameter specifies the
        /// maximum number of TCHARs that can be stored in the output buffer. If
        /// FORMAT_MESSAGE_ALLOCATE_BUFFER is set, this parameter specifies the minimum number of
        /// TCHARs to allocate for an output buffer. For ANSI text, this is the number of bytes; for
        /// Unicode text, this is the number of characters.
        /// </param>
        /// <param name="_arguments">
        /// Pointer to an array of values that are used as insert values in the formatted message. A
        /// %1 in the format string indicates the first value in the Arguments array; a %2 indicates
        /// the second argument; and so on.
        /// </param>
        /// <returns>
        /// If the function succeeds, the return value is the number of TCHARs stored in the output
        /// buffer, excluding the terminating null character. <br></br><br>If the function fails,
        /// the return value is zero. To get extended error information, call Marshal.GetLastWin32Error.</br>
        /// </returns>
        [DllImport("user32.dll", EntryPoint = "FormatMessageA", CharSet = CharSet.Ansi)]
        private static extern int _formatMessage(int _dwFlags, IntPtr _lpSource, int _dwMessageId, int _dwLanguageId, StringBuilder _lpBuffer, int _nSize, int _arguments);
        ...
        /// </summary>
        /// <param name="_hLibModule">
        /// Handle to the loaded DLL module. The LoadLibrary or GetModuleHandle function returns
        /// this handle.
        /// </param>
        /// <returns>
        /// If the function succeeds, the return value is nonzero. <br></br><br>If the function
        ...
        private static extern int _freeLibrary(IntPtr _hLibModule);
        ...
        /// variable. <br></br><br>If the function fails, the return value is NULL. To get extended
        /// error information, call Marshal.GetLastWin32Error.</br>
        /// </returns>
        [DllImport("kernel32.dll", EntryPoint = "GetProcAddress", CharSet = CharSet.Ansi)]
        private static extern IntPtr _getProcAddress(IntPtr _hModule, string _lpProcName);

        /// <summary>
        /// The LoadLibrary function maps the specified executable module into the address space of
        /// the calling process.
        /// </summary>
        /// <param name="_lpLibFileName">
        /// Pointer to a null-terminated string that names the executable module (either a .dll or
        /// .exe file). The name specified is the file name of the module and is not related to the
        /// name stored in the library module itself, as specified by the LIBRARY keyword in the
        /// module-definition (.def) file.
        /// </param>
        /// <returns>
        /// If the function succeeds, the return value is a handle to the module. <br></br><br>If
        /// the function fails, the return value is NULL. To get extended error information, call Marshal.GetLastWin32Error.</br>
        /// </returns>
        [DllImport("kernel32.dll", EntryPoint = "LoadLibraryA", CharSet = CharSet.Ansi)]
        private static extern IntPtr _loadLibrary(string _lpLibFileName);

        /// <summary>
        /// The LookupPrivilegeValue function retrieves the locally unique identifier (LUID) used on
        /// a specified system to locally represent the specified privilege name.
        /// </summary>
        /// <param name="_lpSystemName">
        /// Pointer to a null-terminated string specifying the name of the system on which the
        /// privilege name is looked up. If a null string is specified, the function attempts to
        /// find the privilege name on the local system.
        /// </param>
        /// <param name="_lpName">
        /// Pointer to a null-terminated string that specifies the name of the privilege, as defined
        ...
        /// </param>
        /// <param name="_lpLuid">
        /// Pointer to a variable that receives the locally unique identifier by which the privilege
        ...
        /// token. These requested access types are compared with the token's discretionary
        /// access-control list (DACL) to determine which accesses are granted or denied.
        /// </param>
        /// <param name="_tokenHandle">
        ...
        private static extern int _openProcessToken(IntPtr _processHandle, int _desiredAccess, ref IntPtr _tokenHandle);
        ...
        /// (S4). If the ForceFlag parameter is TRUE, the system suspends operation immediately; if
        /// it is FALSE, the system requests permission from all applications and device drivers
        /// before doing so.
        /// </summary>
        /// <param name="_hibernate">
        /// Specifies the state of the system. If TRUE, the system hibernates. If FALSE, the system
        /// is suspended.
        /// </param>
        /// <param name="_forceCritical">
        /// Forced suspension. If TRUE, the function broadcasts a PBT_APMSUSPEND event to each
        /// application and driver, then immediately suspends operation. If FALSE, the function
        /// broadcasts a PBT_APMQUERYSUSPEND event to each application to request permission to
        /// suspend operation.
        /// </param>
        /// <param name="_disableWakeEvent">
        ...
        {
            IntPtr libPtr = _loadLibrary(library);
            if (!libPtr.Equals(IntPtr.Zero))
            {
                if (!_getProcAddress(libPtr, method).Equals(IntPtr.Zero))
                {
                    _freeLibrary(libPtr);
                    return true;
                }
        ...
        protected static string FormatError(int number)
        {
            ...
            _formatMessage(_fORMAT_MESSAGE_FROM_SYSTEM, IntPtr.Zero, number, 0, buffer, buffer.Capacity, 0);
